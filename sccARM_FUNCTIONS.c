...
{	arm_text();
	...
	arm_label("ENTRY");
	arm_label("main");
	arm_movs(GP, ((DATABASE >> 16) & 0xffff));
	arm_bl_("main");
}
	...
	*/
	...
	{	/* Not a leaf; do full stack frame...
		...
		*/
		...
		arm_addi(SP, SP, 4);		/* sp = old sp */
		arm_jr(RA);			/* return */
		arm_prelabel(n);
		arm_addi(SP, SP, -highwater);	/* sp -= highwater */
		arm_sw(FP, highwater-4, SP);	/* sp[highwater-4] = fp */
		arm_addi(FP, SP, highwater-4);	/* fp = &(sp[highwater-4]) */
